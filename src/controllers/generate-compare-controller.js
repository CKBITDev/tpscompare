...
export default class GenerateCompareController{
    ...
    static async outbound(req,res){
        ...
        for (let index = 0; index < dataNotMatch.length; index++) {
            ...
            let dataJdeNew = [];  
            let dataJdeNewId = [];  
            var dataTps = await executeQuery(`select outbound_header_id,id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan,wk_inout from  outbound_detail d where ${whereTps.join(" and ")} and created_at <= '${tanggal}' order by jml_satuan asc`);
            for (let index = 0; index < dataTps.length; index++) {
                const tps = dataTps[index];
            ...
            for (let index = 0; index < dataJde.length; index++) {
                ...
                if(dataTps2.length > 0){
                    ...
                    // ========= INSERT DATA ======
                    // ====== HEADER ===========
                    const values = [[jde.OTDOC, '3', 'CSMI', jde.IMDSC1, jde.TPCINV, '-', null, '-', '-', 'JDE_IMPORT', 'S', null, 2]]
