...
export default class GenerateCompareController{
    ...
    static async inbound(req,res){
        ...
        for (let index = 0; index < dataNotMatch.length; index++) {
            ...
            var dataTps = await executeQuery(`select inbound_header_id,d.id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from inbound_detail d inner join inbound_header h on h.id = d.inbound_header_id where  ${whereTps.join(" and ")} and d.created_at <= '${tanggal}' and kd_dok = 'Y' order by jml_satuan asc`);
            ...
            //==========
            let indexing = 0;
            var dataTps = await executeQuery(`select d.id as id_detail,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from inbound_detail d inner join inbound_header h on h.id = d.inbound_header_id where  ${whereTps.join(" and ")}and kd_dok = 'Y' and d.created_at <= '${tanggal}' order by jml_satuan asc`);
            var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn}) order by OTQTY asc`); 
            //menampung data jde
    ...
    static async outbound(req,res){
        ...
        const dataMatch = await executeQuery(queryNotMatchOutbound);
        const dataNotMatch = dataMatch.filter(match => match.compare == "NO")
        let tpsId = [];
        let tpsDetailId = [];
        let jdeId = [];
        ...
        for (let index = 0; index < dataNotMatch.length; index++) {
            ...
            queryNotMatchJdeIn = `('${notMatch.jde_item_code}','${notMatch.no_bc11}','${notMatch.no_pos_bc11}','3')`;
            let dataTpsNew = [];
            let dataJdeNew = [];  
            var dataTps = await executeQuery(`select outbound_header_id,id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from  outbound_detail d where ${whereTps.join(" and ")} and created_at <= '${tanggal}' order by jml_satuan asc`);
            var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn})  order by OTQTY asc`); 
            let indexing = 0;
            for (let index = 0; index < dataTps.length; index++) {
                const tps = dataTps[index];
                ...
                if(dataJde.length > 0){
                    ...
                    tpsId.push(tps.outbound_header_id)
                    tpsDetailId.push(tps.id)
                    dataJdeNew.push({id:null})
                }
            }
            dataNotMatch[index].fromTPS = {tps:dataTps,jde:dataJdeNew};
            if(tpsId.length > 0){
                await executeQuery(`delete from outbound_header where id in(${tpsId.join(",")})`); 
            }

            if(tpsDetailId.length > 0){
                await executeQuery(`delete from outbound_detail where id in(${tpsDetailId.join(",")})`); 
            }

            var dataTps = await executeQuery(`select d.id as id_detail,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from outbound_detail d where  ${whereTps.join(" and ")}and created_at <= '${tanggal}' order by jml_satuan asc`);
            var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn}) order by OTQTY asc`); 
            //menampung data jde
            for (let index = 0; index < dataJde.length; index++) {
                const jde = dataJde[index];
                let dataTps2 = await executeQuery(`select id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from  outbound_detail d where d.jde_item_code = '${jde.jde_item_code_jde}' and d.no_bc11 = '${jde.no_bc11_jde}' and d.no_pos_bc11 = '${jde.no_pos_bc11_jde}' and d.wk_inout = '${jde.wk_inout_jde}' and d.jml_satuan = ${parseFloat(jde.jml_satuan_jde)} and created_at <= '${tanggal}' order by jml_satuan asc`);   
                if(dataTps2.length > 0){
                    dataTpsNew.push(dataTps2[0]);
                    indexing++;
                }else{
                    // ========= INSERT DATA ======
                    // ====== HEADER ===========
                    const values = [[jde.OTDOC, '3', 'CSMI', jde.IMDSC1, jde.TPCINV, '-', null, '-', '-', 'FTP', 'S', null, 2]]
                    const result = await executeQuery('insert into outbound_header( `doc_no`, `kd_dok`, `kd_tps`, `nm_angkut`, `no_voy_flight`, `call_sign`, `tgl_tiba`, `kd_gudang`, `ref_number`, `insert_from`, `status`, `user_id`, `company_profile_id`) VALUES (?)',values); 
                    const idRes = result.insertId;
                    // ======  HEADER ===========
                    // ====== DETAIL ===========
                    const valuesDetail = [
                    ...
                    await executeQuery('insert into outbound_detail( `outbound_header_id`, `seri_out`, `no_bl_awb`, `tgl_bl_awb`, `id_consignee`, `consignee`, `no_bc11`, `tgl_bc11`, `no_pos_bc11`, `no_tangki`, `jml_satuan`, `jns_satuan`, `kd_dok_inout`, `no_dok_inout`, `tgl_dok_inout`, `wk_inout`, `kd_sar_angkut_inout`, `no_pol`, `pel_muat`, `pel_transit`, `pel_bongkar`, `jde_co_doc_ty`, `jde_item_code`, `jde_related_doc`, `future_use_01`, `future_use_04`, `future_use_07`) VALUES (?)',valuesDetail); 
                    // ====== DETAIL ===========
                    // ========= INSERT DATA ======
                    jdeId.push(jde.id);
                    dataTpsNew.push({id:null})
    ...
    static async adjustmentX(req,res){
        ...
        for (let index = 0; index < dataNotMatch.length; index++) {
            ...
            let dataTpsNew = [];
            let dataJdeNew = [];  
            var dataTps = await executeQuery(`select adjustment_header_id,d.id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from adjustment_detail d inner join adjustment_header h on h.id = d.adjustment_header_id where  ${whereTps.join(" and ")}and d.created_at <= '${tanggal}' and kd_dok = 'X' order by jml_satuan asc`);
            var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn})  order by OTQTY asc`); 
            let indexing = 0;
            ...
            var dataTps = await executeQuery(`select d.id as id_detail,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from adjustment_detail d inner join adjustment_header h on h.id = d.adjustment_header_id where  ${whereTps.join(" and ")}and kd_dok = 'X' and d.created_at <= '${tanggal}' order by jml_satuan asc`);
            var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn}) order by OTQTY asc`); 
            //menampung data jde
            for (let index = 0; index < dataJde.length; index++) {
                const jde = dataJde[index];
    ...
    static async adjustmentY(req,res){
            ...
            for (let index = 0; index < dataNotMatch.length; index++) {
                ...
                let dataTpsNew = [];
                let dataJdeNew = [];  
                var dataTps = await executeQuery(`select adjustment_header_id,d.id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from adjustment_detail d inner join adjustment_header h on h.id = d.adjustment_header_id where  ${whereTps.join(" and ")}and d.created_at <= '${tanggal}' and kd_dok = 'Y' order by jml_satuan asc`);
                var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn})  order by OTQTY asc`); 
                let indexing = 0;
                ...
                var dataTps = await executeQuery(`select d.id as id_detail,jde_item_code,no_bc11,no_pos_bc11,jml_satuan from adjustment_detail d inner join adjustment_header h on h.id = d.adjustment_header_id where  ${whereTps.join(" and ")}and kd_dok = 'Y' and d.created_at <= '${tanggal}' order by jml_satuan asc`);
                var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn}) order by OTQTY asc`); 
                //menampung data jde
                for (let index = 0; index < dataJde.length; index++) {
                    const jde = dataJde[index];
