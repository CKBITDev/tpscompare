...
export default class GenerateCompare2Controller{
    ...
    static async outbound(req,res){
        ...
        for (let index = 0; index < dataNotMatch.length; index++) {
            ...
            var dataJdeNew = [];
            var dataTps = await executeQuery(`select outbound_header_id,d.id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan,wk_inout from outbound_detail d left join outbound_header oh on d.outbound_header_id = oh.id  where ${whereTps.join(" and ")} and d.created_at <= '${tanggal}' and oh.id is null order by jml_satuan asc`);
            //console.log(`select outbound_header_id,d.id,jde_item_code,no_bc11,no_pos_bc11,jml_satuan,wk_inout from outbound_detail d left join outbound_header oh on d.outbound_header_id = oh.id  where ${whereTps.join(" and ")} and d.created_at <= '${tanggal}' and oh.id is null order by jml_satuan asc`)
            for (let index = 0; index < dataTps.length; index++) {
                const tps = dataTps[index];
                console.log(parseFloat(tps.jml_satuan));
                var dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where OTLITM = '${tps.jde_item_code}' and OTBC11 = '${tps.no_bc11}' and OTPOSN = '${tps.no_pos_bc11}' and OTDCCD = '3' and OTDIOT = '${tps.wk_inout}' and OTQTY = '${parseFloat(tps.jml_satuan)}'  order by OTQTY asc`); 
                if(dataJde.length == 0){
