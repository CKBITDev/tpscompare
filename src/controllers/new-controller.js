...
export default class NewController{
    ...
    static async outbound(req,res){
        ...
        for (let index = 0; index < dataNotMatch.length; index++) {
            ...
            const dataJde = await executeQuery(`select id,OTLITM as jde_item_code_jde,OTBC11 as no_bc11_jde,OTPOSN as no_pos_bc11_jde,OTQTY as jml_satuan_jde,OTDIOT AS wk_inout_jde,jde_tps.* from jde_tps where(OTLITM,OTBC11,OTPOSN,OTDCCD) in (${queryNotMatchJdeIn}) order by OTQTY asc`); 
            //menampung data jde
            ...
            dataNotMatch[index].fromJDE = {jde:dataJde};
        }
        console.log(dataNotMatch.length)
